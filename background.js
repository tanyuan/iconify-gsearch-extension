...
function iconify () {
    ...
    for (var i = 0; i < navs.length; i++) {
        ...
        switch(text) {
            case "圖片":
                var imgURL = chrome.extension.getURL("images/photo_24px.svg");
                nav.className += " iconify";
                nav.style.backgroundImage = "url(" + imgURL + ")";
                break;
            case "影片":
                var imgURL = chrome.extension.getURL("images/videocam_24px.svg");
                nav.className += " iconify";
                nav.style.backgroundImage = "url(" + imgURL + ")";
                break;
            case "地圖":
                var imgURL = chrome.extension.getURL("images/place_24px.svg");
                nav.className += " iconify";
                nav.style.backgroundImage = "url(" + imgURL + ")";
                break;
            case "新聞":
                var imgURL = chrome.extension.getURL("images/receipt_24px.svg");
                nav.className += " iconify";
                nav.style.backgroundImage = "url(" + imgURL + ")";
                break;
        }
